...
IF DEFINED KUDU_SELECT_PYTHON_VERSION_CMD (
  ...
  SET /P PYTHON_RUNTIME=<"%DEPLOYMENT_TEMP%\__PYTHON_VER.tmp"
  IF !ERRORLEVEL! NEQ 0 goto error

  SET PYTHON_VER=%PYTHON_RUNTIME:~10,3%

  SET PYTHON_EXE=%SYSTEMDRIVE%\python%PYTHON_VER%\python.exe
  SET PYTHON_ENV_MODULE=venv

  IF NOT EXIST "%PYTHON_EXE%" (
    echo Python version %PYTHON_VER% was not found
    goto error
  )
...
IF NOT EXIST "%DEPLOYMENT_TARGET%\env\azure.env.%PYTHON_VER%.txt" (
  IF EXIST "%DEPLOYMENT_TARGET%\requirements.txt" (
    echo Creating %PYTHON_VER% virtual environment.
    %PYTHON_EXE% -m %PYTHON_ENV_MODULE% env
    IF !ERRORLEVEL! NEQ 0 goto error

    echo %PYTHON_VER%> "env\azure.env.%PYTHON_VER%.txt"
  ) ELSE (
...
for /f %%i in ('dir /b env\Lib\site-packages\azure*') do (
    echo Installing %%i package
    SETLOCAL ENABLEDELAYEDEXPANSION
    IF NOT "!ERRORLEVEL!"=="0" (
        echo %%i package install failed
        goto error
    )
)
