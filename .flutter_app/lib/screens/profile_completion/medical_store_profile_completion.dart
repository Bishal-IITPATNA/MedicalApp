...
class MedicalStoreProfileCompletion extends StatefulWidget {
  const MedicalStoreProfileCompletion({super.key});

  @override
  State<MedicalStoreProfileCompletion> createState() => _MedicalStoreProfileCompletionState();
}

class _MedicalStoreProfileCompletionState extends State<MedicalStoreProfileCompletion> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      final response = await _apiService.put('/api/medical-store/profile', {
      ...
      if (mounted) {
        if (response['success']) {
          Navigator.pushReplacementNamed(context, '/medical-store-dashboard');
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    Text('Store Information', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: 'Store Address', prefixIcon: Icon(Icons.home), border: OutlineInputBorder()),
                      maxLines: 2,
                      validator: (value) => value == null || value.isEmpty ? 'Please enter store address' : null,
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
