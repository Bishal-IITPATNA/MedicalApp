...
# Maximum allowed prescription payload size (~3 MB raw).
MAX_PRESCRIPTION_BYTES = 3 * 1024 * 1024
ALLOWED_PRESCRIPTION_MIMES = {
    'image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'application/pdf',
}


def _extract_prescription(data: dict):
    """
    Pull `prescription_image` (base64 data URL) and `prescription_filename`
    from a request body and validate them.

    Returns (image_data_url, filename, uploaded_at) or (None, None, None)
    when the payload is absent.

    Raises ValueError on validation failure.
    """
    raw = data.get('prescription_image')
    if not raw:
        return None, None, None

    if not isinstance(raw, str) or not raw.startswith('data:'):
        raise ValueError('prescription_image must be a base64 data URL')

    try:
        header, b64 = raw.split(',', 1)
    except ValueError:
        raise ValueError('Malformed prescription_image data URL')

    mime = header.split(';')[0].replace('data:', '').strip().lower()
    if mime not in ALLOWED_PRESCRIPTION_MIMES:
        raise ValueError(f'Unsupported prescription type: {mime}')

    # base64 length -> approx raw bytes
    approx_bytes = (len(b64) * 3) // 4
    if approx_bytes > MAX_PRESCRIPTION_BYTES:
        raise ValueError('Prescription file too large (max 3 MB)')

    filename = (data.get('prescription_filename') or '').strip()[:255] or None
    return raw, filename, datetime.utcnow()
...
def medicine_orders():
    ...
    # Optional patient-uploaded prescription
    try:
        rx_image, rx_filename, rx_uploaded_at = _extract_prescription(data)
    ...
    try:
        ...
        if selected_store_id:
            ...
            order = MedicineOrder(
                ...
                otp_verified=False,
                prescription_image=rx_image,
                prescription_filename=rx_filename,
                prescription_uploaded_at=rx_uploaded_at,
            )
...
# ----------------------------------------------------------------------
@bp.route('/medicine-orders/<int:order_id>/prescription', methods=['GET'])
@jwt_required()
def get_medicine_order_prescription(order_id: int):
    """Return the data URL of the prescription attached to a medicine order."""
    current_user_id = get_jwt_identity()
    patient = Patient.query.filter_by(user_id=current_user_id).first()
    if not patient:
        return jsonify({'error': 'Patient profile not found'}), 404

    order = MedicineOrder.query.filter_by(id=order_id, patient_id=patient.id).first()
    if not order:
        return jsonify({'error': 'Order not found'}), 404
    if not order.prescription_image:
        return jsonify({'error': 'No prescription attached to this order'}), 404

    return jsonify({
        'success': True,
        'order_id': order.id,
        'prescription_image': order.prescription_image,
        'prescription_filename': order.prescription_filename,
        'prescription_uploaded_at': order.prescription_uploaded_at.isoformat()
            if order.prescription_uploaded_at else None,
    }), 200


@bp.route('/lab-orders/<int:order_id>/prescription', methods=['GET'])
@jwt_required()
def get_lab_order_prescription(order_id: int):
    """Return the data URL of the prescription attached to a lab test order."""
    current_user_id = get_jwt_identity()
    patient = Patient.query.filter_by(user_id=current_user_id).first()
    if not patient:
        return jsonify({'error': 'Patient profile not found'}), 404

    order = LabTestOrder.query.filter_by(id=order_id, patient_id=patient.id).first()
    if not order:
        return jsonify({'error': 'Order not found'}), 404
    if not order.prescription_image:
        return jsonify({'error': 'No prescription attached to this order'}), 404
